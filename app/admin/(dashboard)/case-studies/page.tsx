...
import {listAdminCaseStudies} from '@/lib/store'
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10">
                <div>
                    ...
                    <p className="text-sm text-white/50">
                        {studies.length} case studies — {publishedCount} pubblicati, {draftCount} bozze
                    </p>
                </div>
                <Link
            ...
            {studies.length === 0 ? (
                ...
                <div className="relative border border-white/10 bg-[#0a0a0a]">
                    ...
                    <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/10 text-[10px] font-mono uppercase tracking-[0.25em] text-white/40">
                        <div className="col-span-5">Titolo / Cliente</div>
                        <div className="col-span-2">Settore</div>
                    ...
                    {studies.map(s => (
                        <div key={s.id} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 last:border-b-0 items-center hover:bg-white/[0.02] transition-colors">
                            <div className="col-span-5">
                                <Link href={`/admin/case-studies/${s.id}/edit`} className="block group">
                                    <p className="font-vcr text-sm leading-tight group-hover:text-white text-white/90 transition-colors">{s.title}</p>
                                    <p className="text-[10px] font-mono text-white/30 mt-1">{s.client} · /{s.slug}</p>
                                </Link>
                            </div>
                            <div className="col-span-2 text-xs font-mono text-white/60">{s.industry}</div>
                            ...
                            <div className="col-span-1 text-right flex items-center justify-end gap-3">
                                <Link href={`/admin/case-studies/${s.id}/edit`}
                                      className="text-[10px] font-mono uppercase tracking-wider text-white/50 hover:text-white transition-colors">
                                    Edit
                                </Link>
                                {s.published && (
                                    <Link href={`/case-studies/${s.slug}`} target="_blank" rel="noopener"
                                          aria-label="Apri case study pubblicato"
                                          className="text-[10px] font-mono uppercase tracking-wider text-white/50 hover:text-white transition-colors">
                                        ↗
                                    </Link>
                                )}
...
function EmptyState() {
    return (
        <div className="relative border border-dashed border-white/15 bg-[#0a0a0a]/50 p-12 text-center">
            ...
            <p className="font-vcr text-2xl mb-4">Nessun case study</p>
            <p className="text-xs font-mono text-white/40 max-w-md mx-auto mb-6">
                Crea il primo case study per iniziare a popolare il portfolio.
            </p>
